...
import numpy as np
from joblib import dump, hash, load
...
class Defaults:
    ...
    PERIOD_BTW_SUMMARY_PERFS = (
        -1
    )  # Set to -1 for avoiding call to env.summarizePerformance
...
if __name__ == "__main__":
    ...
    if parameters.param1 is not None and parameters.param1 != "1":
        ...
        parameters.steps_per_epoch = (
            parameters.steps_per_epoch
            - (parameters.steps_per_epoch % (24 * 4 * int(parameters.param1)))
            - 1
        )

    # --- Instantiate environment ---
    env = MG_two_storages_env(
        rng, parameters.param1, parameters.param2, parameters.param3
    )
    ...
    agent.attach(
        bc.FindBestController(
            validationID=env.VALIDATION_MODE, testID=env.TEST_MODE, unique_fname=fname
        )
    )
