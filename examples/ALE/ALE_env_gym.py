...
class MyEnv(Environment):
    ...
    def __init__(self, rng, **kwargs):
        ...
        self._reduced_screen = cv2.resize(
            self._screen, (84, 84), interpolation=cv2.INTER_LINEAR
        )
    ...
    def act(self, action):
        ...
        for t in range(4):
            ...
            self._reduced_screen = cv2.resize(
                self._screen, (84, 84), interpolation=cv2.INTER_NEAREST
            )  # 84*84
    ...
    def observe(self):
        return [(np.array(self.state) - 128.0) / 128.0]
