...
from tensorflow.keras.layers import (Activation, Conv2D, Dense, Flatten, Input,
                                     Layer, MaxPooling2D, Permute, Reshape,
                                     concatenate)
    ...
    def _buildDQN(self):
        ...
        for i, dim in enumerate(self._input_dimensions):
            ...
            if len(dim) == 3 or len(dim) == 4:
                ...
                x = Conv2D(8, (4, 4), activation="relu", padding="valid")(
                    x
                )  # Conv on the frames
                x = Conv2D(16, (3, 3), activation="relu", padding="valid")(
                    x
                )  # Conv on the frames
            ...
            # - observation[i] is a VECTOR
                ...
                if dim[0] > 3:
                    ...
                    inputs.append(input)
                    reshaped = Reshape(
                        (dim[0], dim[1], 1), input_shape=(dim[0], dim[1])
                    )(input)
            ...
            # - observation[i] is a SCALAR -
                ...
                if dim[0] > 3:
                    ...
                    x = Conv2D(8, (1, 2), activation="relu", padding="valid")(
                        reshaped
                    )  # Conv on the history
                    x = Conv2D(8, (1, 2), activation="relu", padding="valid")(
                        x
        ...
        params = [param for layer in layers for param in layer.trainable_weights]
